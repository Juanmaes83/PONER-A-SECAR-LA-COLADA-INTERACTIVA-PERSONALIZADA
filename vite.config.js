...
const V51_EXTERNAL_API=`function mountExternal(root,animations=[],name='External 3D',report={}){if(customRoot)scene.remove(customRoot);if(mixer)mixer.stopAllAction();clipMap.clear();activeClip='';customRoot=root;customRoot.name='V5_1_EXTERNAL_ASSET';customRoot.traverse(o=>{if(o.isMesh){o.castShadow=true;o.receiveShadow=true}});const box=new THREE.Box3().setFromObject(customRoot),size=new THREE.Vector3(),center=new THREE.Vector3();box.getSize(size);box.getCenter(center);const ss=4/Math.max(size.y,.001);customRoot.scale.multiplyScalar(ss);customRoot.position.set(-center.x*ss,-box.min.y*ss-2,-center.z*ss);scene.add(customRoot);if(animations?.length){mixer=new THREE.AnimationMixer(customRoot);for(const c of animations)clipMap.set((c.name||'').toLowerCase(),c);playClip('idle')}state.template='custom';persist();syncTemplateSelect();syncVisibility();customRoot.userData.externalAsset={name,report};return report}\nfunction clearExternal(){if(customRoot){scene.remove(customRoot);customRoot=null}if(mixer){mixer.stopAllAction();mixer=null}clipMap.clear();activeClip='';if(state.template==='custom'){state.template='aya';persist();if(!rig){rig=createRig(TEMPLATES.aya);scene.add(rig.root)}syncTemplateSelect();syncVisibility()}}\nwindow.HangingMediaV51Character={mountExternal,clearExternal,loadCustom};\n${V51_BOOT}`;
