  ...
  plugins: [{
    ...
    transform(code,id) {
      ...
      if (clean.endsWith('/src/v5-1-character-library.js')) {
        ...
        code=mustReplace(code,"function tick(now=performance.now()){if(renderer&&rig){poseRig(now);","function tick(now=performance.now()){if(renderer&&rig){if(window.HMSInteractionRouter?.blocks?.('character')){dragging=false;targetEffort=.035;effort=.035;narrative='idle';wheelUntil=0;releaseAt=0;satisfactionUntil=0;if(canvas)canvas.dataset.state='idle'}poseRig(now);",'pause character narrative during 3D edit');
