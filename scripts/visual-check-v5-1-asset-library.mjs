...
await page.waitForFunction(()=>window.__V51_ASSET_LIBRARY__?.ready&&window.__V51_MULTI_IMPORT_FIX__?.ready,null,{timeout:8000});
const bootDiag=await page.evaluate(()=>({library:!!window.HangingMediaV51AssetLibrary,ready:window.__V51_ASSET_LIBRARY__?.ready||false,multiImport:window.__V51_MULTI_IMPORT_FIX__?.ready||false,character:!!window.HangingMediaV51Character,getSceneContext:typeof window.HangingMediaV51Character?.getSceneContext,panel:!!document.querySelector('#v5-1-asset-library'),status:document.querySelector('#v51al-status')?.textContent||'',charCanvas:!!document.querySelector('#v5-1-character-canvas')}));
await fs.writeFile('artifacts/asset-library-boot-diagnostic.json',JSON.stringify({bootDiag,consoleErrors,pageErrors},null,2));
if(!bootDiag.ready||!bootDiag.multiImport)throw new Error(`Asset Library boot failed: ${JSON.stringify({bootDiag,consoleErrors,pageErrors})}`);
await page.setInputFiles('#v51al-upload',['artifacts/library-red.zip','artifacts/library-blue.zip']);
await page.evaluate(()=>window.__V51_MULTI_IMPORT_FIX__.wait());
await page.waitForFunction(()=>window.__V51_ASSET_LIBRARY__?.assets?.length===2,null,{timeout:8000});
const importDiag=await page.evaluate(()=>({assets:window.__V51_ASSET_LIBRARY__?.assets||[],apiAssets:window.HangingMediaV51AssetLibrary?.getAssets?.().map(a=>({id:a.id,name:a.name,format:a.format}))||[],status:document.querySelector('#v51al-status')?.textContent||'',cards:document.querySelectorAll('.v51al-card').length}));
...
if(importDiag.assets.length!==2)throw new Error(`Expected 2 assets after multi-ZIP import: ${JSON.stringify({importDiag,consoleErrors,pageErrors})}`);
