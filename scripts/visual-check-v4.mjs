...
await page.waitForSelector('.v4-rope-link');
...
const before=await page.evaluate(()=>({
 ...
 hasRopeLink:Boolean(document.querySelector('.v4-rope-link')),
...
await page.mouse.move(box.x+box.width*.28,box.y+box.height*.5,{steps:12});
...
const during=await page.evaluate(()=>({
 ...
 pullAmount:Number(document.querySelector('#v4-character-layer')?.dataset.pullAmount||0),
 layerTransform:document.querySelector('#v4-character-layer')?.style.transform||'',
 armTransform:document.querySelector('.v4-raised-arm')?.getAttribute('transform')||'',
 ropeTransform:document.querySelector('.v4-rope-link')?.getAttribute('transform')||''
}));
await page.screenshot({path:'artifacts/hanging-media-v4-pull.png',fullPage:true});
await page.mouse.up();
await page.waitForTimeout(650);
const after=await page.evaluate(()=>({state:document.querySelector('#v4-character-layer')?.dataset.state,pullAmount:Number(document.querySelector('#v4-character-layer')?.dataset.pullAmount||0)}));
...
if(!before.characterVisible||!before.hasRig||!before.hasRaisedArm||!before.hasRopeLink)throw new Error('Narrative character rig incomplete');
...
if(during.pullAmount<0.25)throw new Error(`Pull response too weak: ${during.pullAmount}`);
if(!during.armTransform||during.armTransform===before.armTransform)throw new Error('Raised pull arm did not articulate');
if(!during.ropeTransform)throw new Error('Rope-link tension transform missing');
