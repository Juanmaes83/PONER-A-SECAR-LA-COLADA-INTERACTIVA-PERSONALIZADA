...
const saved=await page.evaluate(async()=>await window.HangingMediaV51Phase1Persistence.flush('qa-explicit'));
const savedInstance=saved.find(x=>x.id===id);
if(!savedInstance||Math.abs(savedInstance.rotation.y-67*Math.PI/180)>.035)throw new Error(`3D rotation was not committed before reload: ${JSON.stringify({id,saved})}`);
...
const restoredInstance=restored.instances.find(x=>x.id===id);
await fs.writeFile('artifacts/v5-1-phase1-report.json',JSON.stringify({id,videoResult,rotation,saved,restored,errors},null,2));
...
if(!restoredInstance||Math.abs(restoredInstance.ry-67*Math.PI/180)>.035)throw new Error(`3D rotation persistence failed: ${JSON.stringify({id,savedInstance,restored:restored.instances})}`);
await page.screenshot({path:'artifacts/v5-1-phase1-final.png',fullPage:true});
if(errors.length)throw new Error(`Browser errors: ${errors.join(' | ')}`);
console.log(JSON.stringify({ok:true,id,videoResult,rotation,savedInstance,restored},null,2));
await browser.close();
